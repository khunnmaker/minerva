...
import { Flower2, LogIn, Loader2, AlertTriangle, ArrowLeft } from 'lucide-react';
import { groupLogins, memberAvatar, teamAvatar, type GroupMeta } from '@pantheon/ui';
import { getLogins, login, setSession, type Agent, type LoginCard } from './lib/api';

const PIN_LEN = 6;

// Role-grouped, tap-to-drill-down, Metro-tile login picker — the SAME UX as the Pantheon portal
// and every other suite app, adapted to Mali's jasmine-green accent. Unlike Vesta (supervisor
// only) or Jupiter (gated to a role list), Mali is an ALL-STAFF app — every account that can
// authenticate may sign in here, so there is no post-login role check.
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans text-slate-800">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-md w-full p-6">
        <div className="flex items-center gap-2 text-green-700 mb-1">
          <Flower2 size={24} />
          <h1 className="text-xl font-bold">มะลิ — ผู้ช่วยความรู้</h1>
        </div>
        <p className="text-sm text-slate-500 mb-5">คลังความรู้ทีมงาน · เลือกชื่อเพื่อเข้าสู่ระบบ</p>
        ...
        {view === 'list' ? (
                ...
                {selected.kind === 'password' ? (
                  <>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 mb-3 rounded-md border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-green-400"
                    />
                    <button
                      onClick={() => submit(selected.email, secret)}
                      disabled={busy || !secret}
                      className="w-full px-3 py-2.5 rounded-md bg-green-600 hover:bg-green-700 text-white text-sm font-semibold flex items-center justify-center gap-1 disabled:opacity-50"
                    >
                  ...
                  <>
                    ...
                    <input
                      ...
                      placeholder="••••••"
                      className="w-full px-3 py-2 mb-1 rounded-md border border-slate-300 text-center tracking-[0.5em] text-lg focus:outline-none focus:ring-2 focus:ring-green-400"
                    />
                    <div className="h-5 flex items-center justify-center">
                      {busy && <Loader2 size={16} className="animate-spin text-green-500" />}
                    </div>
                  </>
                )}
          ...
          <>
            ...
            <input
              ...
              className="w-full px-3 py-2 mb-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-green-400"
            />
            <label className="block text-xs font-semibold text-slate-500 mb-1">รหัสผ่าน</label>
            <input
              type="password"
            ...
            <button
              ...
              disabled={busy}
              className="w-full px-3 py-2 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold flex items-center justify-center gap-1 disabled:opacity-50"
            >
              {busy ? <Loader2 size={16} className="animate-spin" /> : <LogIn size={16} />} เข้าสู่ระบบ
            </button>
            {!loadFailed && (
        ...
        <p className="text-[10px] text-slate-300 mt-4">มะลิเปิดให้พนักงานทุกคน — ความรู้ทั้งบริษัทอยู่ที่เดียว</p>
      </div>
    </div>
  );
}
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1 px-2 py-2 -ml-2 mb-3 rounded-md text-sm font-medium text-slate-500 hover:text-green-600 hover:bg-slate-100"
    >
