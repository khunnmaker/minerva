...
> now know from Prominent's existing **Minerva** and **Vulcan** systems. Diana =
...
2. **Vulcan is built and live.** Channa's **Open Decision #1 ("does Vulcan have an API / how to sync
   stock — the whole architecture depends on it")** is **essentially RESOLVED:** Vulcan shares a
...
3. **A single source of truth for catalog + price + stock already exists** — one Postgres `Product`
   table that Minerva and Vulcan both use. Channa's SSOT guardrail (§8 "avoid two catalogs") is
...
- **Vulcan** (LIVE, same monorepo + same DB) — stock manager. The stock manager imports Express's
...
truth already exists** (Minerva + Vulcan on shared Postgres), there's a third option:
...
1. ✅ **Vulcan / stock integration — RESOLVED.** Shared Postgres + daily-fresh `Product.stock` + a
...
1. Confirm Diana's **read path to the shared catalog/stock** (DB vs API) — gated by the platform choice (§5), not by Vulcan anymore.
...
7. **Live stock** from the shared DB / Vulcan (already fresh daily).
...
## 9. Relationship map — Diana ↔ Minerva ↔ Vulcan
- **Catalog + price + photos:** the shared Postgres `Product` table (Minerva-owned, Vulcan-updated). Diana **reads** it. Photos via `/content/product/:sku`.
- **Stock:** Vulcan keeps `Product.stock`/`stockAt` fresh daily; Diana shows it live.
...
1. Walk Mike through the **platform fork (§5)** — emphasize that the stock-integration blocker is gone
   (Vulcan/shared DB exists), so the choice is now maintenance appetite + payments + pricing model.
2. Resolve **payment model (#2)** and **pricing structure (#4)**.
3. If **custom (B):** decide repo layout — same monorepo as Minerva/Vulcan (share the `Product` table
...
- **One source of truth** for product/price/stock = the shared Postgres `Product` (Minerva/Vulcan). Diana reads it; don't fork a new catalog.
- If Diana shares the DB: **ADD only**, never alter Minerva/Vulcan columns; keep one migrator. Secrets in env only, never committed.
...
```
Reference docs in this repo: docs/VULCAN_BRIEF.md, docs/VULCAN_DEPLOY.md, and api/prisma/schema.prisma (the Product model).
```
